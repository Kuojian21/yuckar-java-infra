...
public class MapperFuture<V, T> implements Future<T> {
	...
	public static <V, T> Future<T> wrap(Future<V> future, ThrowableFunction<V, T, Exception> wrapper) {
		return new MapperFuture<>(future, wrapper);
	}

	private final Future<V> future;
	private final ThrowableFunction<V, T, Exception> function;

	public MapperFuture(Future<V> future, ThrowableFunction<V, T, Exception> wrapper) {
		super();
		this.future = future;
