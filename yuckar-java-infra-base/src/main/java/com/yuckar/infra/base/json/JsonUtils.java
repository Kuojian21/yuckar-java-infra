package com.yuckar.infra.base.json;
...
import com.yuckar.infra.base.scanner.Scanner;
...
public class JsonUtils {
	...
	public static <E> Set<E> fromSetJson(String json, Class<E> valueType) {
		if (StringUtils.isEmpty(json)) {
			return null;
		}
		try {
			return MAPPER.readValue(json, defaultInstance().constructCollectionType(Set.class, valueType));
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
	}

	public static <E> Set<E> fromSetJson(byte[] bytes, Class<E> valueType) {
		if (bytes == null) {
			return null;
		}
		try {
			return MAPPER.readValue(bytes, defaultInstance().constructCollectionType(Set.class, valueType));
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
	}

	public static <E> Set<E> fromSetJson(InputStream inputStream, Class<E> valueType) {
		if (inputStream == null) {
			return null;
		}
		try {
			return MAPPER.readValue(inputStream, defaultInstance().constructCollectionType(Set.class, valueType));
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
	}
